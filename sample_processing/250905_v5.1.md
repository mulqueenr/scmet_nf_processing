...
summary(kismet@metadata$cg_cov/kismet@metadata$cov)
#   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# 0.5743  0.7182  0.7686  0.7657  0.8173  0.8925 
...
dat <- combineObject(objList = c(scale_celllines,kismet))
...
obj<-subsetObject(obj,cells=row.names(obj@metadata[obj@metadata$cov>quantile(),])) #number set by 3rd Q of kismet
...
pdf("cellline_cor_10kbp.pdf")
Heatmap(as.matrix(cor_mat))
dev.off()

summary(as.data.frame(obj$sum_matrix)[,c(4:11)])
